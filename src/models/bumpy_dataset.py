...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': torch.from_numpy(image),
                'lin_coms': torch.from_numpy(lin_coms),
                'ang_coms': torch.from_numpy(ang_coms),
...
class BumpyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with rosbag.Bag(file_, 'r') as f:
            ...
            for topic, msg, t in f.read_messages(topics=["/cam1/image_rect/compressed"]):
                if i == file_idx:
                    ...
                    #tensor_img = transform(rgb_img) # Convert the image to PyTorch tensor
                    break
                i=i+1
        ...
        return sample['image'], sample['lin_coms'], sample['ang_coms'], sample['imu_data']

#Some code to test the dataset is working properly
#dataset = BumpyDataset("data/processed/data.csv","data/processed", transform=transforms.Compose([ToTensor()]))
#dataloader = DataLoader(dataset, batch_size=1)
#dataloader_iter = iter(dataloader)

#for i in range(1):
#    img, lin, ang, imu = next(dataloader_iter)

#print(img.size()) #([1, 3, 732, 1490])
#print(imu.size()) #([1, 1, 8])
