...
def getAngularZ():
    global current_idx
    options = [-0.4, -0.3, -0.2, -0.1, 0, 0, 0, 0.1, 0.2, 0.3, 0.4]
    step = random.choice([-1, 1])
    new_idx = current_idx + step
    if new_idx < 0 or new_idx > (len(options)-1):
        angular_z = options[current_idx]
    else:
        angular_z = options[new_idx]
        current_idx = new_idx

    return angular_z

def generatePaths(nrofpaths = 5):
    ...
    for i in range(nrofpaths):
        ...
        current_idx = 5
        for i in range(8):
            linear_x = random.uniform(boundaries[0], boundaries[1])
            angular_z = getAngularZ()
            path.append([linear_x, angular_z])
        pathlist.append(torch.tensor([path]).cuda())
